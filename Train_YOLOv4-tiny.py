...
command = f"C:\darknet\darknet.exe detector train {data_file} {config_file} {pretrained_weights} -dont_show -map -gpus 0 -batch {batch_size} -max_batches {max_batches} -learning_rate {learning_rate} -learning_rate_decay {learning_rate_decay} -learning_rate_decay_steps {learning_rate_decay_steps} -momentum {momentum} -out {output_dir}"
#os.system(command)
print(command)
